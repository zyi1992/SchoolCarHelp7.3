...
@interface GetTheNewStartPlaceViewViewController ()<UISearchBarDelegate, UISearchDisplayDelegate, UITableViewDataSource, UITableViewDelegate>
...
@implementation GetTheNewStartPlaceViewViewController
...
- (void)search:(id)searchRequest error:(NSString *)errInfo
{
    ...
    UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"获取位置信息失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
    [alert show];
}
...
- (void)mapView:(MAMapView *)mapView didAddAnnotationViews:(NSArray *)views
{
    ...
    if (((NSString*)(id <MAAnnotation>)view.annotation.subtitle).length == 0) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:text forKey:ThestartAnnotationCoordinatetitle];
    }else
    {
...
- (void)onGeocodeSearchDone:(AMapGeocodeSearchRequest *)request response:(AMapGeocodeSearchResponse *)response
{
    ...
    [[NSUserDefaults standardUserDefaults]setDouble:[annotations[0] coordinate].latitude forKey:ThestartAnnotationCoordinatelatitude];
    [[NSUserDefaults standardUserDefaults] setDouble:[annotations[0] coordinate].longitude forKey:ThestartAnnotationCoordinatelongitude];
    [[NSUserDefaults standardUserDefaults] synchronize];
...
- (void)initSearchBar
{
    ...
    self.searchBar.placeholder  = @"查找你的起点";
...
- (void)viewDidLoad
{
    ...
    resevrBtn.frame = CGRectMake(0, 0, 80, 40);
    
    //[resevrBtn setImage:[UIImage imageNamed:@"back.png"] forState:UIControlStateNormal];
    [resevrBtn setTitle:@"选择终点" forState:UIControlStateNormal];
    [resevrBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [resevrBtn addTarget:self action:@selector(tranToFindWway) forControlEvents:UIControlEventTouchUpInside];
